	...
	for (std::list<CHAT_WINDOW_ENTRY>::iterator entry = m_ChatWindowEntries.end(); entry != m_ChatWindowEntries.begin(); entry--)
	{
		...
		case CHAT_TYPE_CHAT:
			...
			if (entry->szNick[0] != 0)
			{
				...
				else sprintf(buff, "%s %s", entry->szNick, entry->utf8Message);

				RenderText(buff, pos.x, pos.y, entry->dwNickColor);
			}

			if (pGUI->timestamp && entry->szNick[0] == 0)
			{
				sprintf(buff, "[%s] %s", entry->time, entry->utf8Message);
				RenderText(buff, pos.x, pos.y, entry->dwTextColor);
			}
			break;
...
void CChatWindow::RenderText(const char* u8Str, float posX, float posY, uint32_t dwColor)
{
	...
	while (*textCur)
	{
		...
		if (textCur[0] == '{' && ((&textCur[7] < textEnd) && textCur[7] == '}'))
		{
			...
			{
				// ---> Set alpha -- -- -- -- --
			/*	if (elapsed.count() > 5000  && !m_bIsOpened && !pDialogWindow->m_bIsActive)
				{
					const auto shade = elapsed.count() - 5000;
					uint32_t alpha;

					if (shade < 1024)
						alpha = (1024 - shade) / 8 << 24;
					else
						alpha = 0x00000000;
