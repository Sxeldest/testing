...
CNetGame::CNetGame(const char* szHostOrIp, int iPort, const char *szPlayerName, const char* szPass)
{
	...
	if (pUI) {
		char szStartMsg[64];
		sprintf(szStartMsg, "{FFFFFF}SA-MP {B9C9BF}%s {FFFFFF}Started", pSettings ? pSettings->Get().szSampVersion : SAMP_VERSION);
		pUI->chat()->addDebugMessage(szStartMsg);
	}
}
// 0.3.7
CNetGame::~CNetGame()
...
void CNetGame::Packet_AuthKey(Packet *pkt)
{
	...
	//if (pUI) pUI->chat()->addDebugMessage("Auth key received, generating response...");
...
{
	RakNet::BitStream bsSuccAuth(pkt->data, pkt->length, false);
	PLAYERID MyPlayerID;
	...
	int iVersion = 4057; // 0.3.7
	uint8_t byteMod = 0x01; // Port samparz: Identify as PC to avoid Bad Mod Version
	unsigned int uiChallengeResponse = uiChallenge ^ iVersion;

	uint8_t byteAuthBSLen = strlen(AUTH_BS);
	const char* szVersion = pSettings ? pSettings->Get().szSampVersion : SAMP_VERSION;
	uint8_t byteClientVerLen = strlen(szVersion);
	uint8_t byteNameLen = strlen(pPlayerPool->GetLocalPlayerName());

	RakNet::BitStream bsSend;
	bsSend.Write(iVersion);
	bsSend.Write(byteMod);
	bsSend.Write(byteNameLen);
	bsSend.Write(pPlayerPool->GetLocalPlayerName(), byteNameLen);
	bsSend.Write(uiChallengeResponse);
	bsSend.Write(byteAuthBSLen);
	bsSend.Write(AUTH_BS, byteAuthBSLen);
	bsSend.Write(byteClientVerLen);
	bsSend.Write(szVersion, byteClientVerLen);
